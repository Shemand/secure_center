    ...
    if _computer.isDallas():
        if _row is not None:
            if _row['DallasServers_id'] is None:
                ...
                if _computer.dallas_status != database.DallasStatus.get_last(_computer.name)['type']\
                   if database.DallasStatus.get_last(_computer.name) else _computer.dallas_status:
                   database.ARMs.change_dallas_status(_row, _computer.dallas_status)
